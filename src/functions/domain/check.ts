...
export async function checkDomain(domain: string) {
	...
	const dbdata = await query('SELECT * FROM scams.domains WHERE domain = $1', [
		domain,
	]);
	...
	const checkFishFishAPI = await axios.get(
		"https://api.fishfish.gg/v1/domains/${domain}",
		{
      ...
      2
		...
		return {
			...
			externalApiResponses: {
				fishfishAPI: `${checkFishFishAPI.data.category} ~ ${checkFishFishAPI.data.description}`,
